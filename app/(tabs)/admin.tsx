...
export default function AdminScreen() {
  ...
  async function handleCreate() {
    ...
    setLoading(true);
    try {
      await db.createUserAdmin(
        username.trim(),
        password,
        role,
        unidadOrg.trim() || null,
        cargoField.trim() || null,
      );
      setUsername('');
      setPassword('');
      setRole('user');
    ...
    <View key={item.username} style={styles.userRow}>
      <View>
        ...
        <Text style={styles.userRole}>
          {item.role} · {item.unidad_organica || '-'} · {item.cargo || '-'}
        </Text>
      </View>
      <View style={{ flexDirection: 'row' }}>
        <TouchableOpacity
          onPress={() => router.push(`/usuarios?user=${item.username}`)}
          style={{ marginRight: 8 }}
        >
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.card}>
        ...
        <FlatList
          ...
          keyExtractor={(i, idx) => i.username ?? String(idx)}
        />
      </View>
    </SafeAreaView>
  );
